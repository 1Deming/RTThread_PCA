 ...
 * Change Logs:
 * Date           Author       Notes
 * 2020-06-09     deming       the first version
 */

#ifndef SPI_OLCD_INCLUDED
#define SPI_OLCD_INCLUDED
...
void drv_oled_pwr_en(void);

void drv_oled_pwr_dis(void);

void drv_oled_cs_en(void);

void drv_oled_cs_dis(void);

void drv_oled_rest_en(void);

void drv_oled_rest_dis(void);

void drv_oled_write_command_mode(void);

void drv_oled_write_data_mode(void);
...
//解锁命令
void drv_oled_conmmand_unlock(void);

//锁定之后只能接收解锁命令
void drv_oled_command_lock(void);

//buff = 0xae sleep on; buff = 0xaf sleep off
void drv_oled_sleep_mode(uint8_t buff);

void drv_oled_sleep_on(void);

void drv_oled_sleep_off(void);
...
void drv_oled_select_default_linear_gray_scale_table(void);
...
void drv_oled_deinit(void);

void drv_oled_clear_all_screen(void);

void drv_oled_display_all_screen(void);
...
//enable write RAM 
void drv_oled_write_RAM_en(void);

//write the data
void drv_oled_write_byte(uint8_t data);
void drv_oled_write_bytes(uint8_t *data, int data_size);
void drv_oled_all_pixels_on(void);
void drv_oled_all_pixels_off(void);
