...
----------------------------------------------------------------------
File        : GUI_X_OS.C
Purpose     : This file provides emWin Interface with RTThread RTOS
---------------------------END-OF-HEADER------------------------------
*/
...
{ 
  ...
  osMutex = rt_mutex_create("UI_Mutex",RT_IPC_FLAG_PRIO);
  if( osMutex == RT_NULL )
  {
    while(1);
  }
  ...
  osSemaphore = rt_sem_create("UI_Sem",0,RT_IPC_FLAG_PRIO);  
  if( osSemaphore == RT_NULL )
  {
    while(1);
  }
}
